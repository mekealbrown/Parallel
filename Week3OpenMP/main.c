...
{
    int vec_limit = n - (n % 4);  // Ensure that the loop iterates in multiples of 4
    double local_sum = 0.0;
  // Parallelize only outer loop
  #pragma omp parallel for reduction(+:local_sum)
  for(int i = 0; i < n; i++){
    for(int j = 0; j < n; j++){
      // thread-private sum to reduce contention
      double local_sum = 0.0;
      
      // SIMD vector registers to accumulate results
      __m256d vec_sum1 = _mm256_setzero_pd();  // First SIMD vector
      __m256d vec_sum2 = _mm256_setzero_pd();  // Second SIMD vector
...
double** allocate_matrix(int n)
{
  ...
  // Now set each row pointer to point to the correct location in the data array
  for (int i = 1; i < n; i++) {
      matrix[i] = matrix[0] + i * n;  // Each row points to the correct start location
  }
  return matrix;
}
...
void benchmark_matrix_multiply(int size) {
    ...
    // Warm-up runs to reduce cold-start effects
